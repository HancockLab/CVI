...
public class BuildPolarizedMatrix {
...
//	private Boolean region = null;
	...
	public void setSnpFile(String filename){
		...
		try {
			...
			folderLyrata = new File("/home/CIBIV/andreaf/lyrata_refseq"); // /Volumes/Aner/dataAugust/lyrata_refseq
			...
			*/
			...
			System.out.println("We got lyrata!!");
			 ...
			 accession = Integer.parseInt(filename); // accession = 0;
/*
			if (accession == 1) {
				for (int c=0; c<5; c++) {
...
*/
        	...
        	*/
			...
			// Now build a char[] with CVI bases for polarization of matches to colombia....
			
			char[] cviBases = new char[numSnps];
				...
				while ( scannerCvi.hasNextLine() ) { 
		        	...
		        	if ( splitSnp1[0].charAt(0) == 'C' && splitSnp1[0].charAt(1) == 'h' && splitSnp1[0].charAt(2) == 'r') {
			        	...
			        	if (maskXor[chr-1].get(posSNP)) {
		        			...
		        			int index=0;
		        			while (chr != checkChr[index]) {
		        				if (chr != checkChr[index]) {
		        					index = index + 1;
		        				}
		        			}
		        			
		        			while (posSNP != positions[index]) {
		        				if (posSNP != positions[index]) {
		        					index = index + 1;
		        				}
		        			}
		        			
			        		cviBases[index] = splitSnp1[6].charAt(0);
			        	}
			        }
				}
			}
			
			// System.out.println("cviBases[]: " + Arrays.toString(cviBases) );
			...
			for (int f=accession; f<accession+1; f++) { // accession+1 // listOfSNPFiles.length
				...
				// Think of matches to colombia...

				for (int c=0; c<5; c++) {
					mask[c].xor(maskXor[c]);
				}
				...
				for (int c=0; c<5; c++) {
					for (int p=0; p<mask[c].length(); p++) {
						if ( mask[c].get(p) ) {
		        			...
		        			int index=0;
		        			while ( (c+1) != checkChr[index]) {
		        				if ( (c+1) != checkChr[index]) {
		        					index = index + 1;
		        				}
		        			}
		        			
		        			while (p != positions[index]) {
		        				if (p != positions[index]) {
		        					index = index + 1;
		        				}
		        			}
				        	...
				        	if (positions[index] == p) {
		        				...
		        				System.out.println("Danger at: " + c + " " + p + " " + positions[index] );
		        				
		        				for (int d=10; d>0; d--) {
			        				System.out.print(positions[index-d] + " ");
		        				}
		        				for (int d=0; d<10; d++) {
			        				System.out.print(positions[index+d] + " ");
		        				}
							...
							*/
				        	...
				        	for (int r=0; r<p; r++) {
				        		if (maskLyrata[c].get(r)) {
				        			indexx = indexx+1;
				        		}
				        	}
				        	
				        	if ( cviBases[index] == lyrataChar[c][indexx] ) {
				        		meraviglia[f].set(index);
				        	}
						}
					}
	...
	public static void main(String[] args) {
		BuildPolarizedMatrix buildPolarizedMatrix = new BuildPolarizedMatrix();
		buildPolarizedMatrix.setSnpFile(args[0]); // args[0]
	}
}
