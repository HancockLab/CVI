...
public class Qtl_overlapSelectionScans {
	...
	public void setFileToSubSet(String matrixName, String numPh, String scanName, String bigSmall, String outFold){
		       	...
		       	if (snp.charAt(0) != '#') {
			       	...
			       	for (int spl=0; spl<splitComment.length; spl++) {
			       		if (splitComment[spl].split("=")[0].equals("scan")) {
			       			...
			       			if (scanName.equals("all") || scan.equals(scanName)) {
		...
		if (bigPheno) {
	    	...
	    	while ( scanner.hasNextLine() ) {
				String snp = scanner.nextLine();
		       	...
		       	for (int spl=0; spl<splitComment.length; spl++) {
		       		/////
		       		//			Do big or small phenotypic categories
		       		/////
		...
		phenotypes[phenotypesMem.length] = "all";
		...
		// String outFold = "/srv/biodata/dep_coupland/grp_hancock/andrea/qtls_specificPheno_2019-11-26";
		...
		boolean candGenes = false;
		boolean funcVar = false;
		boolean singleGenes = true;
		...
		String FileName = "";
		if (candGenes) {
	    		FileName = outFold + "/qtlsOverlap_candGenes_scan-" + scanName + ".txt";
		} else {
			if (funcVar) {
				FileName = outFold + "/qtlsOverlap_functionalVar_scan-" + scanName + ".txt";
			} else {
				if (singleGenes) {
					//FileName = outFold + "/singleGeneOverlap_" + phenotypes[phenoNum] + "_scan-" + scanName + ".txt";
				} else {
					FileName = outFold + "/qtlsOverlap_" + phenotypes[phenoNum] + "_scan-" + scanName + ".txt";
				}
			}
		}
   			
		for (int p =phenoNum; p<phenoNum+1; p++) {
   			BitSet[] mask = new BitSet[5];
   			for (int c=0; c<mask.length; c++) {
   				mask[c] = new BitSet();
   			}
   			
			if (!candGenes && !funcVar && !singleGenes) {	
			...
			if (bigPheno) {
				...
				targetPheno = "phenoSpec";
   			       		...
   			       		if (splitComment[spl].split("=")[0].equals(targetPheno)) {
   			       			...
   			       			if (phenotypes[p].equals(pheno) || phenotypes[p].equals("all")) {			// all to test all phenotypes at once
   			       				int start = Integer.parseInt(splitSnp[3]);
   			       				int end = Integer.parseInt(splitSnp[4]);
   			       				for (int pos=start; pos<=end; pos++) {
			    ...
			    ////
			    //		Now you have the mask
			    //		Change it if you want to check all candidate genes 
			    //		or functional variants
			    ////
			    if (candGenes || funcVar) {
				// Build a mask for all candidate genes
   				//
   				BitSet[] maskC = new BitSet[5];
   				for (int c=0; c<maskC.length; c++) {
   					maskC[c] = new BitSet();
   				}
				////
				...
				File fileMask = null;
				if (funcVar) {
					// With the 2large deletionis full lenght
					fileMask = new File("/home/fulgione/qtls/candGenes_masks_19-10-25/functionalVariants_qtl.txt_allFuncVar_plus2genes_mask_2020-04-11.txt");
					// With only midpoits forthe 2 latreg depetions
					// fileMask = new File("/home/fulgione/qtls/candGenes_masks_19-10-25/functionalVariants_qtl.txt_variants_mask_2020-04-11.txt");
				} else {
					fileMask = new File("/home/fulgione/qtls/candGenes_masks_19-10-25/candidates_qtl.txt_allCandidateGenes_mask_2020-04-11.txt");
				}
			    ...
			    // If you want to check all candidate genes, hijack numPh
			    //
			    if (singleGenes) {
				// For genes plus 3kb before and after
				// File folder = new File("/home/fulgione/qtls/candGenes_masks_19-10-25/");
			    	// For genes plus 10kb before and after
				// 
				// File folder = new File("/home/fulgione/qtls/candGenes_masks_plus10k_2020-05-19/");
				// For genes plus 50kb before and after
				//
				File folder = new File("/home/fulgione/qtls/candGenes_masks_plus50k_2020-05-20/");
				File[] listOfFiles_part = folder.listFiles();

				// Count mask files
				int matchF = 0;
				for (int f=0; f<listOfFiles_part.length; f++) {
					String end = listOfFiles_part[f].getName().split("_")[listOfFiles_part[f].getName().split("_").length - 1];
					if (end.equals("mask.txt")) {
						matchF = matchF + 1;
					}
				}
				File[] listOfFiles = new File[matchF];
				matchF = 0;
				for (int f=0; f<listOfFiles_part.length; f++) {
					String end = listOfFiles_part[f].getName().split("_")[listOfFiles_part[f].getName().split("_").length - 1];
					if (end.equals("mask.txt")) {
						listOfFiles[matchF] = listOfFiles_part[f];
						matchF = matchF + 1;
					}
				}
				...
				if (listOfFiles[phenoNum].isFile()) {
					File fileMask = null;
					fileMask = listOfFiles[phenoNum];
					Scanner scannerMask = new Scanner(fileMask);
					int chrCVI=0;
					String lineMask=null;
					for (int m=0; m<10; m++) {
				...
				geneName=listOfFiles[phenoNum].getName().split("_")[2];
				FileName = outFold + "/" + geneName + "_overlap_scan-" + scanName + ".txt";
			    }
			...
			// System.out.print(phenotypes[p] + "\t" + cardQtl + "\t" + cardXpClr + "\t" + expectOverlap + "\t" + overlapXpClr + "\t" + (double)overlapXpClr/(double)expectOverlap + "\n");
   		    	...
   		    	out.print(geneName + "\t" + expectOverlap + "\t" + overlapXpClr + "\t" + (double)overlapXpClr/(double)expectOverlap + "\n");
   		    	
   		    	//////
   		    	////
			//			If there is an enrichment,
   		    	//			randomise both Qtl regions and selection scan peaks 
   		    	////
   		    	/////
			...
			if ((double)overlapXpClr/(double)expectOverlap > 1) {
	...
	public static void main(String[] args) {
		...
		// qtl_overlapSelectionScans.setFileToSubSet("/srv/biodata/dep_coupland/grp_hancock/andrea/backup_qtls_fromCelia_sortedPheno_allTablesMerged.gff_overlapOut.txt_2019-11-26_overlapOut.txt_doublesOut.txt", args[0], args[1], args[2], args[3]);
		qtl_overlapSelectionScans.setFileToSubSet("/srv/biodata/dep_coupland/grp_hancock/andrea/backup_qtls_fromCelia_sortedPheno_allTablesMerged.gff_overlapOut.txt_2019-11-26_overlapOut.txt_doublesOut_withAngelasPapers.txt", args[0], args[1], args[2], args[3]);
