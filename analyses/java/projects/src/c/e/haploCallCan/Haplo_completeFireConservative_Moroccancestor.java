...
public class Haplo_completeFireConservative_Moroccancestor {
	...
	private Haplo_completeFireConservative_Moroccancestor() {}
	
	public void call_Haplo_completeFireConservative(String matrixName, String canarian) {
		try {
/*
 * 80 genomes are erased in agedHapCalls
 *
...
*/					for (int co=0; co<ids.length; co++) {
	    					// System.out.print(" ids: " + ids[co] + " splitIDs: " + splitIDs[i] + "\n" + idsUnsplit + "\n");
...
*/					for (int co=0; co<ids.length; co++) {
			...
			// Play hard, in the fire
				...
				Writer writer = new FileWriter(whereWe + "hapCalls_YeaCutN_" + can + ".txt");
				...
				int[] memHap = new int[allButCanCviIndexes.length + 1];			// Plus one for private variation to canaries
					...
					int tot =0;
						...
						// Cut or elongate haplotype calls for every pair of samples
							...
							if ( (baseBB == baseCan) && (baseCan != 'N') ) {
								...
								// Initialize hap call, if this is the first informative match
						        	if (memHap[b] == 0) {
								...
								if (memHap[b] != 0) {	
									// maskEndtwo is set true if the previous informative snp was already a mismatch, or a 'N' - then we cut the hap call
									if ( maskEndtwo.get(b) ) {
										...
										if ((memHap[b] >= 2)) {
											...
											// SecondLast records if the second last SNP was a mismatch or a 'N'. 
											// In the 1st case, we counted one new mutation too much in the varWithin - correct it
					        					if (secondLast.get(b)) {
										...
										secondLast.clear(b);
								        	...
								        	varWithin[b] = 0;
					       		 			...
					       		 			// If you got here, this is the first mismatch, so wait one more to cut, and count as new mutation
										maskEndtwo.set(b);
	...
	public static void main(String[] args) {
		Haplo_completeFireConservative_Moroccancestor haplo_completeFireConservative_Moroccancestor = new Haplo_completeFireConservative_Moroccancestor();
		haplo_completeFireConservative_Moroccancestor.call_Haplo_completeFireConservative(args[0], args[1]);
	}
}
