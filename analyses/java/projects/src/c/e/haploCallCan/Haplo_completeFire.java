...
public class Haplo_completeFire {
	...
	private Haplo_completeFire() {}
	
	public void call_haplo_completeFire(String canarian) {
		try {
			...
			*/
	    	...
	    	for (int can=whichCan; can<whichCan + 1; can++) {
				...
				Writer writer = new FileWriter(whereWe + "canHaploCallsFire_varWit_" + can + ".txt"); 
		    	...
		    	while (scannerMatrix.hasNextLine()) {
	        		...
	        		if (baseCan != 'N') {
	        			...
	        			/*
	        			if (Integer.parseInt(splitSnp[1]) <= 773) {
				    		System.out.println("position: " + splitSnp[1]);
				    		System.out.println("baseCan: " + baseCan);
				    		for (int check=2; check<splitIDs.length; check++) {
					    		if (Integer.parseInt(splitIDs[check]) == 6180) {
						    		System.out.println("base out: " + splitSnp[check].charAt(0));
					    		}
					    		if (Integer.parseInt(splitIDs[check]) == 9862) {
						    		System.out.println("base esp: " + splitSnp[check].charAt(0));
					    		}
				    		}
		        		}
	        			*/
	        			...
	        			if (!(inIbp && inCVI)) {
					    	...
					    	// Just distinguish unique stuff to canaries
					    	...
					    	for (int b=0; b<allButCanCviIndexes.length; b++) {
					        				...
					        				// if (!unique) {
					        					if ( maskEndtwo.get(b) ) {
					        						if ((memHap[b] >= 2)) {
					        				    		...
					        				    		out.print(chr + "\t" + startHap[b] + "\t" + endHap[b] + "\t" + Integer.parseInt(splitIDs[allButCanCviIndexes[b]]) + "\t" + countryHap + "\t" +  memHap[b] + "\t" +  (endHap[b] + 1 - startHap[b]) + "\t" + (varWithin[b]-1) + "\n");
				        		...
				        		if (unique) {
	        						...
	        						if ( (memHap[memHap.length-1] > 0) && (!maskEndtwo.get(1000000)) ) {
			        					maskEndtwo.set(1000000);
		        						varWithin[varWithin.length-1] = varWithin[varWithin.length-1] + 1;
	        						}
    	    						if ( (memHap[memHap.length-1] > 0) && (maskEndtwo.get(1000000)) ) {
        					    		out.print(chr + "\t" + startHap[memHap.length-1] + "\t" + endHap[memHap.length-1] + "\t" + 0 + "\t" + "CANISL" + "\t" +  memHap[memHap.length-1] + "\t" +  (endHap[memHap.length-1] + 1 - startHap[memHap.length-1]) + "\t" + (varWithin[varWithin.length-1]-1) + "\n");
	...
	public static void main(String[] args) {
		Haplo_completeFire haplo_completeFire = new Haplo_completeFire();
		haplo_completeFire.call_haplo_completeFire(args[0]);
	}
}
