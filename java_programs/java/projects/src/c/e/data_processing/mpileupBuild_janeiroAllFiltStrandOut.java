...
public class mpileupBuild_janeiroAllFiltStrandOut {
	...
	public void setSnpFile(String filename, String wereWeResults){
		...
		try {
			...
			File snpEffFile = new File(wereWeResults + localName + ".all.snp");
			...
			outSnpEff.println("#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO");
			
			while ( scannerSnpEff.hasNextLine() ) { 
				String snp1 = scannerSnpEff.nextLine();
		       	String[] splitSnp1 = snp1.split("\t");
		       	
		       	if ( splitSnp1[0].charAt(0) == 'C' && splitSnp1[0].charAt(1) == 'h' && splitSnp1[0].charAt(2) == 'r' && splitSnp1[6].charAt(0) != 'N' && splitSnp1[6].charAt(0) != 'S' && splitSnp1[6].charAt(0) != '-') {
			   		int chr = Integer.parseInt(Character.toString(splitSnp1[0].charAt(3)));
			       	int posSNP = Integer.parseInt(splitSnp1[1]);
			       	
			       	outSnpEff.print(chr + "\t" + posSNP + "\t" + "." + "\t" + splitSnp1[2] + "\t" +splitSnp1[6] + "\t" + "." + "\t" +  "PASS" + "\t" + "." + "\n");
				}
			}
			outSnpEff.close();
