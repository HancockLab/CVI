...
public class Jsfs_fromVcf_simulaSlim_plusFst {
	
	public Jsfs_fromVcf_simulaSlim_plusFst() {}
	...
	public void setMatrixFile(String matrixName, String resultsPart, String samplesSanto, String samplesFogo) {
		...
		try {
			...
			int[][] jsfs = new int[2*index1.length + 1][2*index2.length + 1];
		...
		while ( scannerMatrix.hasNextLine() ) {
			...
			if (snp.charAt(0) != '#') {
				...
				int der1 = 0;
					...
					if (splitSnp[index1[s1]].charAt(2) == '1') {
						der1 = der1 + 1;
					}
				}
				// Pop2
				int der2 = 0;
					...
					if (splitSnp[index2[s2]].charAt(2) == '1') {
						der2 = der2 + 1;
					}
				}
				...
				double p1 = (double)der1/(double)(index1.length*2);
				double p2 = (double)der2/(double)(index2.length*2);
			...
			out2.print(fixSsegF + "\t" + fixFsegS + "\t" + snps + "\t" + (double)fixSsegF/(double)snps + "\t" + (double)fixFsegS/(double)snps + "\t" + stats + "\t" + fixFnoS + "\t" + (double)fixFnoS/(double)snps + "\n"); 
			out2.close();
	...
	public static void main(String[] args) {
		Jsfs_fromVcf_simulaSlim_plusFst jsfs_fromVcf_simulaSlim_plusFst = new Jsfs_fromVcf_simulaSlim_plusFst();
		jsfs_fromVcf_simulaSlim_plusFst.setMatrixFile(args[0], args[1], args[2], args[3]);
	}
}
