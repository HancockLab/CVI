...
#
# Read tables
# qtls_ls=as.data.frame(read.table("/Volumes/CVI/final/cvi/scripts_toPublish/percentagePrivate_smallPheno_2020-04-16.txt", header = F))
# one qtl out
qtls_ls=as.data.frame(read.table("./data/percentagePrivate_smallPheno_2020-07-30.txt", header = F))

# genes_ls=as.data.frame(read.table("/Volumes/CVI/final/cvi/scripts_toPublish/percentagePrivate_candGenes_2020-04-16.txt", header = F))
# To modify:
genes_ls=as.data.frame(read.table("./data/percentagePrivate_candGenes_2020-07-30.txt", header = F))

mean(na.omit(qtls_ls$V4))
mean(na.omit(genes_ls$V4))

mean(sum(qtls_ls$V2)/(sum(qtls_ls$V2)+sum(qtls_ls$V3)))
mean(sum(genes_ls$V2)/(sum(genes_ls$V2)+sum(genes_ls$V3)))
...
# mean.genes_plot=sum(genes_ls$V1)/sum(genes_ls$V2)
...
###
# From: qtls_genes_var.command
# Genomewide segrega: 3214 fixed: 416252	ratio:0.0076621227942193166
gw.segr = 3214
gw.snps = 3214+416252
...
# pdf("/Volumes/CVI/final/cvi/qtl/fig_longShortBranchLog10_14.pdf", height=6,width=6)
...
# colOr=c(rep(rgb(178/255, 34/255, 34/255, 0.3), 2), rgb(255/255, 0/255, 0/255, 0.8), rgb(255/255, 20/255, 147/255, 0.8))
...
# axis(1, at=c(1.1, 1.3, 1.5), tick=F, labels=c("QTL \n regions", "Candidate \n genes", "Functional \n variants"), line=1.2)
...
# abline(h=log(offset+0.0076621227942193166, base=base), lwd=0.1)
...
# points(x=x[[1]], y=log(mean(na.omit(qtls_plot)) + offset, base=base), bg = col[[1]], pch=symb[[1]], col="black", lwd=0.1, cex=2.5)
...
# hist(na.omit(log(variants_plot+0.00001, base=base)))
...
# points(x=c(x[[1]], x[[2]], x[[3]]), y=c(logQtlMean, meanGenes, meanVar), col = col[[1]], type="l", lwd=0.5)
